...
unsigned int count_triangles() {
  ...
  #pragma omp parallel reduction(+:delta) num_threads(32)
  {
    // #pragma omp parallel for
    int id = omp_get_thread_num();
    int p = omp_get_num_threads();
    //The outer loop traverses over all possible vertices (y). The iteration starts with vertex #1
    for(unsigned int i = 1 + id; i<N-1; i += p) {					
      // Find X
      unsigned int *curr_row_x = IA+i; // X index
      unsigned int *curr_row_A = IA+i+1; // Z index
      unsigned int num_nnz_curr_row_x = *curr_row_A - *curr_row_x; // num of neighbors of Y
      unsigned int *x_col_begin = (JA + *curr_row_x); // First neighbor of Y in JA
      unsigned int *x_col_end = x_col_begin;
      unsigned int *row_bound = x_col_begin + num_nnz_curr_row_x; // Bound after the Y's last neighbor
      unsigned int col_x_min = 0; // valid lower bound for x
      unsigned int col_x_max = i-1; // valid higher bound for x
      ...
      unsigned int y_col_first = i + 1; // First valid higher neighbor
      unsigned int x_col_first = 0;
      unsigned int *y_col = y_col_begin; // Z vertex in Y's higher neighbor list
      ...
      for(unsigned int j = 0; j < num_nnz_y; ++j,++y_col) {
        ...
        unsigned int row_index_A = *y_col - y_col_first; 		// Converts z's absolute vertex number to relative index for accessing IA array
        unsigned int *x_col = x_col_begin;			
        unsigned int num_nnz_A = *(curr_row_A + row_index_A + 1) - *(curr_row_A + row_index_A);
        unsigned int *A_col = (JA + *(curr_row_A + row_index_A)); // Start of Z's neighbors			
        unsigned int *A_col_max = A_col + num_nnz_A; // End of Z's neighbors
